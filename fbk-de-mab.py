...
def solve(f, lb, ub, dim, temp=-1, num_clusters=-1, T=-1, M_factor=0, gen_mult=1, generate_strategy=generate_old):  # ,pop_hint=-1
    CR = 0.9  # MAGIC
...
if __name__ == "__main__":
    ...
    if test:
        ...
        k = eval(sys.argv[1])
        phi_g = eval(sys.argv[2])
        gen_strat = eval(sys.argv[3])
        temp = eval(sys.argv[4])
        T = eval(sys.argv[5])
        M_factor = eval(sys.argv[6])
